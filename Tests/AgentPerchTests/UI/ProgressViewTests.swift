...
final class ProgressViewTests: XCTestCase {
    ...
    func testStartingMidTurnDatesTheTurnFromTheFirstToolCallSeen() {
        // AgentPerch launched after the prompt went in — "at least this old" beats no clock at all.
        var progress = SessionProgress()
    ...
    func testAnUpdateForATaskWeNeverSawCreatedChangesNothing() {
        // AgentPerch attached mid-session: the create was never seen, so the id addresses nothing
