...
/// asking for the item rather than AgentPerch. Spawned from a GUI app with no TTY that read can be
...
/// means macOS shows the ordinary "AgentPerch wants to access key … in your keychain" dialog with
...
final class RuntimeUsageTokenSource: UsageTokenSource, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.agentperch", category: "usage-credentials")
    private let queue = DispatchQueue(label: "com.agentperch.usage-credentials")
