    ...
    private func row(_ todo: ProgressTodo) -> some View {
        ...
        return HStack(alignment: .firstTextBaseline, spacing: 6) {
            Text(glyph(for: todo.state))
                .foregroundStyle(isDone ? Color.perchGreen : Color.perchGray)
                .frame(width: 9, alignment: .leading)
            Text(todo.text)
                .strikethrough(isDone, color: Color.perchGray.opacity(0.7))
                .foregroundStyle(Color.perchGray.opacity(isDone ? 0.55 : 1))
                .lineLimit(1)
                .truncationMode(.tail)
            Spacer(minLength: 0)
        }
