...
struct FeaturedSessionCard: View {
    ...
    var body: some View {
        Button(action: onClick) {
            HStack(spacing: 13) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    if let subtitle {
                        Text("You: \(subtitle)")
                            .font(.system(size: 11))
                            .foregroundStyle(Color.perchGray)
                            .lineLimit(1)
                            .truncationMode(.tail)
                    }
            ...
            .padding(14)
            .background(Color.perchCard, in: RoundedRectangle(cornerRadius: 16))
    ...
    private var statusLine: some View {
        ...
        case .done:
            Text("Done — click to jump")
                .foregroundStyle(Color.perchGreen)
    ...
    private var statusColor: Color {
        ...
        case .needsAction: .perchAmber
        case .active, .working: .perchGreen
        case .idle, .done, .ended: .perchGray
        }
    }
...
struct CompactSessionRow: View {
    ...
    var body: some View {
        Button(action: onClick) {
            ...
            .frame(minHeight: 36)
            .background(Color.perchCard.opacity(0.72), in: RoundedRectangle(cornerRadius: 11))
        ...
        VStack(alignment: .leading, spacing: 11) {
            ...
            if let diff = request.diff {
                ...
                Text(request.details)
                    ...
                    .padding(10)
                    .background(Color.perchPanel, in: RoundedRectangle(cornerRadius: 9))
            }
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .background(Color.perchPanel, in: Capsule())
    ...
    private func blockView(_ block: PlanMarkdown.Block) -> some View {
        ...
        case let .listItem(marker, text):
            ...
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                Text(marker)
                    .font(.system(size: 12, weight: .medium))
                    .foregroundStyle(Color.perchBlue)
                Text(Self.rendered(text))
                    .font(.system(size: 12))
                    .fixedSize(horizontal: false, vertical: true)
            }
...
struct ResolvedActionCard: View {
    ...
    var body: some View {
        ...
        case let .answered(label):
            row(dot: .perchGreen, text: label)
                .accessibilityElement(children: .combine)
                .accessibilityLabel(label)
        case .failed:
            Button(action: onJump) {
                row(dot: .perchAmber, text: "Couldn't answer — click to jump")
                    .contentShape(RoundedRectangle(cornerRadius: 16))
            }
            .buttonStyle(.plain)
        ...
        VStack(alignment: .leading, spacing: 1) {
            ForEach(Array(diff.lines.enumerated()), id: \.offset) { _, line in
                Text("\(line.kind == .removed ? "-" : "+")\(line.text)")
                    .font(.system(size: 11, design: .monospaced))
                    .foregroundStyle(line.kind == .removed ? Color.red : Color.perchGreen)
                    .padding(.horizontal, 8)
                    .padding(.vertical, 2)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .background(
                        (line.kind == .removed ? Color.red : Color.perchGreen).opacity(0.11)
                    )
            }
...
private struct ActionButtons: View {
    ...
    var body: some View {
        HStack {
            ...
            // kept for when AgentPerch itself is active; CardShortcutMonitor covers the normal
            ...
            Button(negativeTitle) { onDecision(.deny) }
                ...
                .padding(.vertical, 7)
                .foregroundStyle(.white)
                .background(Color.perchBadge, in: Capsule())
