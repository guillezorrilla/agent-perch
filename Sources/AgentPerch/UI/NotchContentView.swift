...
struct CompactLeadingView: View {
    ...
    private var aggregateColor: Color {
        if store.sessions.contains(where: { $0.status == .needsAction }) { return .perchAmber }
        ...
        Group {
            if !store.sessions.isEmpty {
                Text("\(store.sessions.count)")
                    ...
                    .background(Color.perchBadge, in: RoundedRectangle(cornerRadius: 6))
                    .contentShape(Rectangle())
                    .onTapGesture { actions.expand() }
                    .onHover { if $0 { actions.hoverExpand() } }
                ...
                ForEach(Array(lines.enumerated()), id: \.offset) { index, line in
                    HStack(spacing: 7) {
                        ForEach(Array(line.enumerated()), id: \.offset) { position, window in
                            if position > 0 {
                                Text("|")
                                    .foregroundStyle(Color.perchGray.opacity(0.65))
                            }
                            windowView(window)
                        }
