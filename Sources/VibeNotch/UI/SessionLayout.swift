...
enum SessionLayout {
    ...
    static func split(sessions: [AgentSession], cap: Int = maxFullCards) -> Split {
        ...
        guard !busy.isEmpty else {
            ...
            // most recent session, so the panel is never just a bare list of rows. An IDE
            // workspace row (Antigravity, Cursor) is never eligible for that card — it is capped
            // at `.idle` in its source (never `isBusy`) precisely so it can't be mistaken for a
            // real session, and the exemption has to be repeated here too: it would otherwise
            // still win the fallback slot merely by being the single most recently touched
            // folder (#29, #11).
            guard let mostRecent = sessions.first(where: { !$0.isIDEWorkspace }) else {
